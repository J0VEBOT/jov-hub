import {
  type JOVConfigSpec,
  type JovisSkillMetadata,
  JovisSkillMetadataSchema,
  isTextContentType,
  type NixPluginSpec,
  parseArk,
...
export type { JovisSkillMetadata, SkillInstallSpec }
...
export function parseJOVisMetadata(frontmatter: ParsedSkillFrontmatter) {
  const metadata = getFrontmatterMetadata(frontmatter)
  const metadataRecord =
    metadata && typeof metadata === 'object' && !Array.isArray(metadata)
      ? (metadata as Record<string, unknown>)
      : undefined
  const jovbotMeta = metadataRecord?.jovbot
  const jovisMeta = metadataRecord?.jovis
  const J0VEBOTMeta = metadataRecord?.J0VEBOT
  const metadataSource =
    jovbotMeta && typeof jovbotMeta === 'object' && !Array.isArray(jovbotMeta)
      ? (jovbotMeta as Record<string, unknown>)
      : jovisMeta && typeof jovisMeta === 'object' && !Array.isArray(jovisMeta)
        ? (jovisMeta as Record<string, unknown>)
        : J0VEBOTMeta && typeof J0VEBOTMeta === 'object' && !Array.isArray(J0VEBOTMeta)
          ? (J0VEBOTMeta as Record<string, unknown>)
          : undefined
  const jovisRaw = metadataSource ?? frontmatter.jovis
  if (!jovisRaw || typeof jovisRaw !== 'object' || Array.isArray(jovisRaw)) return undefined

  try {
    const jovisObj = jovisRaw as Record<string, unknown>
    const requiresRaw =
      typeof jovisObj.requires === 'object' && jovisObj.requires !== null
        ? (jovisObj.requires as Record<string, unknown>)
        : undefined
    const installRaw = Array.isArray(jovisObj.install) ? (jovisObj.install as unknown[]) : []
    const install = installRaw
      .map((entry) => parseInstallSpec(entry))
      .filter((entry): entry is SkillInstallSpec => Boolean(entry))
    const osRaw = normalizeStringList(jovisObj.os)

    const metadata: JovisSkillMetadata = {}
    if (typeof jovisObj.always === 'boolean') metadata.always = jovisObj.always
    if (typeof jovisObj.emoji === 'string') metadata.emoji = jovisObj.emoji
    if (typeof jovisObj.homepage === 'string') metadata.homepage = jovisObj.homepage
    if (typeof jovisObj.skillKey === 'string') metadata.skillKey = jovisObj.skillKey
    if (typeof jovisObj.primaryEnv === 'string') metadata.primaryEnv = jovisObj.primaryEnv
    if (typeof jovisObj.cliHelp === 'string') metadata.cliHelp = jovisObj.cliHelp
    ...
    const nix = parseNixPluginSpec(jovisObj.nix)
    if (nix) metadata.nix = nix
    const config = parseJOVConfigSpec(jovisObj.config)
    if (config) metadata.config = config

    return parseArk(JovisSkillMetadataSchema, metadata, 'JOVis metadata')
  } catch {
    return undefined
  }
}
...
function parseJOVConfigSpec(input: unknown): JOVConfigSpec | undefined {
  if (!input || typeof input !== 'object') return undefined
  const raw = input as Record<string, unknown>
