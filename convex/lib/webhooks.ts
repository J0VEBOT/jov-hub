...
function buildDescription(event: WebhookEvent, skill: WebhookSkillPayload) {
  ...
  if (event === 'skill.highlighted') return 'Newly highlighted skill on JovHub.'
  if (skill.version) return `New version v${skill.version} published on JovHub.`
  return 'New skill published on JovHub.'
}
