...
describe('webhook config', () => {
  ...
  it('defaults site url when missing', () => {
    ...
    process.env.DISCORD_WEBHOOK_URL = 'https://example.com'
    const config = getWebhookConfig()
    expect(config.siteUrl).toBe('https://hub.j0ve.bot')
  })
})
...
describe('payload building', () => {
  it('builds canonical url with owner', () => {
    ...
    expect(url).toBe('https://hub.j0ve.bot/KrauseFx/beeper')
  })
  ...
  it('builds a publish embed', () => {
    const payload = buildDiscordPayload(
      ...
      { url: 'https://example.com', highlightedOnly: false, siteUrl: 'https://hub.j0ve.bot' },
    )
