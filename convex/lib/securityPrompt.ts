...
export const SECURITY_EVALUATOR_SYSTEM_PROMPT = `You are a security evaluator for JOV AI skills. Users install skills to extend what their AI agent can do. Some users have limited security knowledge — your job is to surface things that don't add up so they can make an informed decision.
...
export function assembleEvalUserMessage(ctx: SkillEvalContext): string {
  ...
  const rawJOVis = (ctx.parsed.jovis ?? {}) as Record<string, unknown>
  const meta = (ctx.parsed.metadata ?? {}) as Record<string, unknown>
  const J0VEBOTFallback =
    meta.J0VEBOT && typeof meta.J0VEBOT === 'object' && !Array.isArray(meta.J0VEBOT)
      ? (meta.J0VEBOT as Record<string, unknown>)
      : {}
  const jovis = Object.keys(rawJOVis).length > 0 ? rawJOVis : J0VEBOTFallback
  const requires = (jovis.requires ?? J0VEBOTFallback.requires ?? {}) as Record<string, unknown>
  const install = (jovis.install ?? []) as Array<Record<string, unknown>>
...
- Published: ${new Date(ctx.createdAt).toISOString()}`)
  ...
  // Flags
  const always = fm.always ?? jovis.always
  const userInvocable = fm['user-invocable'] ?? jovis.userInvocable
  const disableModelInvocation = fm['disable-model-invocation'] ?? jovis.disableModelInvocation
  const os = jovis.os
  sections.push(`**Flags:**
...
- OS restriction: ${Array.isArray(os) ? os.join(', ') : formatWithDefault(os, 'none')}`)
  ...
  const primaryEnv = (jovis.primaryEnv as string | undefined) ?? 'none'
