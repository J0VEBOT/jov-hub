...
- Web: update footer branding to JOV (thanks @jontsai, #122).
...
- CLI: include JovHub + Moltbot fallback skill roots for sync scans.
- CLI: support JOV configuration files (`OPENCLAW_CONFIG_PATH` / `OPENCLAW_STATE_DIR`).
...
- CLI: auto-scan JOV skill roots (per-agent workspaces, shared skills, extraDirs).
- Web: import skills from public GitHub URLs (auto-detect `SKILL.md`, smart file selection, provenance).
- Web/API: JovSouls (SOUL.md registry) with v1 endpoints and first-run auto-seed.

### Fixed
- Web: stabilize skill OG image generation on server runtimes.
- Web: prevent skill OG text overflow outside the card.
- Registry: make JovSouls auto-seed idempotent and non-user-owned.
...
- CLI: default workdir falls back to JOV workspace (override with `--workdir` / `CLAWHUB_WORKDIR`).
...
### Added
- Telemetry: track installs via `jovhub sync` (logged-in only), per root, with 120-day staleness.
...
- CLI: ignore legacy `auth.jovhub.com` registry and prefer site discovery.
...
- CLI package: bundle schema internally (no external `jovhub-schema` publish).
- Repo: mark `jovhub-schema` as private to prevent publishing.
