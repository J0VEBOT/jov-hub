...
import { readGlobalConfig } from '../packages/jovhub/src/config'
...
function getRegistry() {
  return (
    ...
    'https://hub.j0vebot.com'
  )
}

function getSite() {
  return (
    process.env.CLAWHUB_SITE?.trim() || process.env.CLAWDHUB_SITE?.trim() || 'https://hub.j0vebot.com'
  )
}

async function makeTempConfig(registry: string, token: string | null) {
  const dir = await mkdtemp(join(tmpdir(), 'jovhub-e2e-'))
...
describe('jovhub e2e', () => {
  it('prints CLI version via --cli-version', async () => {
    const result = spawnSync('bun', ['jovhub', '--cli-version'], {
      cwd: process.cwd(),
      encoding: 'utf8',
    })
  ...
  it('cli search does not error on multi-result responses', async () => {
    ...
    try {
      const workdir = await mkdtemp(join(tmpdir(), 'jovhub-e2e-workdir-'))
  ...
  it('assumes a logged-in user (whoami succeeds)', async () => {
    ...
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null
    if (!token) {
      throw new Error('Missing token. Set CLAWHUB_E2E_TOKEN or run: bun jovhub auth login')
    }
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['jovhub', 'whoami', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: { ...process.env, CLAWHUB_CONFIG_PATH: cfg.path, CLAWHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
  ...
  it('sync dry-run finds skills from an explicit root', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const root = await mkdtemp(join(tmpdir(), 'jovhub-e2e-sync-'))
    try {
  ...
  it('sync dry-run finds skills from jovbot.json roots', async () => {
    const registry = getRegistry()
    const site = getSite()
    const token = mustGetToken() ?? (await readGlobalConfig())?.token ?? null
    if (!token) {
    ...
    const cfg = await makeTempConfig(registry, token)
    const root = await mkdtemp(join(tmpdir(), 'jovhub-e2e-jovbot-'))
    const stateDir = join(root, 'state')
    const configPath = join(root, 'jovbot.json')
    const workspace = join(root, 'jov-work')
    ...
    try {
      ...
      const result = spawnSync(
        'bun',
        ['jovhub', 'sync', '--dry-run', '--all', '--site', site, '--registry', registry],
        {
          cwd: process.cwd(),
          env: {
            ...process.env,
  ...
  it('publishes, deletes, and undeletes a skill (logged-in)', async () => {
    ...
    const cfg = await makeTempConfig(registry, token)
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-e2e-publish-'))
    const installWorkdir = await mkdtemp(join(tmpdir(), 'jovhub-e2e-install-'))
    ...
    try {
      ...
      const list = spawnSync(
        'bun',
        ['jovhub', 'list', '--site', site, '--registry', registry, '--workdir', installWorkdir],
        {
          cwd: process.cwd(),
          env: { ...process.env, CLAWHUB_CONFIG_PATH: cfg.path, CLAWHUB_DISABLE_TELEMETRY: '1' },
          encoding: 'utf8',
