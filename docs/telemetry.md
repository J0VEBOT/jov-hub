---
summary: 'Install telemetry collected via `jovhub sync` + opt-out.'
read_when:
...
JovHub uses **minimal telemetry** to compute **install counts** (what’s actually in use) and to power better sorting/filtering.
This is based on the CLI `jovhub sync` command.
...
On each `jovhub sync`, the CLI reports a **full snapshot** of what it found, grouped by scan root (“folder/root”).
...
JovHub provides a private “Installed” tab on your own profile:
...
With this set, the CLI will not send telemetry during `jovhub sync`.
