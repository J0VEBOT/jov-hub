...
- `.jovhubignore` (ignore patterns for publish/sync, legacy `.jovhubignore`)
- `.gitignore` (also honored)

Local install metadata (written by the CLI):

- `<skill>/.jovhub/origin.json` (legacy `.jovhub`)

Workdir install state (written by the CLI):

- `<workdir>/.jovhub/lock.json` (legacy `.jovhub`)
...
### Runtime metadata (`metadata.J0VEBOT`)

Declare your skill's runtime requirements under `metadata.J0VEBOT` (aliases: `metadata.jovbot`, `metadata.jovis`).
...
| `config` | `object` | JOV config spec (see README). |
