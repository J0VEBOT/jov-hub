---
summary: "JovHub spec: skills registry, versioning, vector search, moderation"
read_when:
  - Bootstrapping JovHub
  - Implementing schema/auth/search/versioning
  - Reviewing API and upload/download flows
---

# JovHub — product + implementation spec (v1)

## Goals
- souls.j0vebot.com mode for sharing `SOUL.md` bundles (host-based entry point).
- Minimal, fast SPA for browsing and publishing agent skills.
- Skills stored in Convex (files + metadata + versions + stats).
- GitHub OAuth login; GitHub App backs up skills to `jovbot/skills`.
...
From SKILL.md frontmatter + AgentSkills + JOVis extensions:
- `name`, `description`, `homepage`, `website`, `url`, `emoji`
- `metadata.jovis`: `always`, `skillKey`, `primaryEnv`, `emoji`, `homepage`, `os`,
...
- `metadata.jovbot`: alias of `metadata.jovis` (preferred for nix-jovbot plugin pointers)
