...
CLI package: `packages/jovhub/` (published as `jovhub`, bin: `jovhub`).
...
- macOS: `~/Library/Application Support/jovhub/config.json`
...
- Headless: `jovhub login --token clh_...`
...
- Reads `<workdir>/.jovhub/lock.json` (legacy `.jovhub`).
...
- Auto-adds JOV skill roots when `~/.jovbot/jovbot.json` is present:
  - `agent.workspace/skills` (main agent)
  - `routing.agents.*.workspace/skills` (per-agent)
  - `~/.jovbot/skills` (shared)
