...
## `jovhub login` opens browser but never completes
...
- It scans:
  - workdir first
  - then fallback roots (legacy `~/jovis/skills`, `~/jovbot/skills`, etc.)
- Provide explicit roots:

```bash
jovhub sync --root /path/to/skills
```
...
- Options:
  ...
  - overwrite: `jovhub update <slug> --force`
  - publish as fork: copy to new folder/slug then `jovhub publish ... --fork-of upstream@version`
