...
Install a skill into `./skills/<slug>` (if JOV is configured, installs into that workspace instead):

```bash
bun jovhub install <slug>
bun jovhub list
```
...
```bash
bun jovhub install <slug> --workdir /tmp/jovhub-demo --dir skills
```

Update:

```bash
bun jovhub update --all
```
...
```bash
mkdir -p /tmp/jovhub-skill-demo && cd /tmp/jovhub-skill-demo
cat > SKILL.md <<'EOF'
---
name: Demo Skill
...
```bash
bun jovhub sync --all --dry-run --no-input
```
