...
- CLI: `packages/jovhub/` (published as `jovhub`, legacy `jovhub`).
- Shared schemas/routes: `packages/schema/` (`jovhub-schema`).
...
- Persist install state:
  - `./.jovhub/lock.json` (per workdir, legacy `.jovhub`)
  - `./skills/<slug>/.jovhub/origin.json` (per skill folder, legacy `.jovhub`)
