...
export function getSiteName(mode: SiteMode = getSiteMode()) {
  return mode === 'souls' ? 'JovSouls' : 'JovHub'
}

export function getSiteDescription(mode: SiteMode = getSiteMode()) {
  return mode === 'souls'
    ? 'JovSouls — the home for SOUL.md bundles and personal system lore.'
    : 'JovHub — a fast skill registry for agents, with vector search.'
}

export function getSiteUrlForMode(mode: SiteMode = getSiteMode()) {
  return mode === 'souls' ? getOnlyCrabsSiteUrl() : getJovHubSiteUrl()
}
