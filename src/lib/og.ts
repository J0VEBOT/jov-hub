import { getJovHubSiteUrl, getOnlyCrabsSiteUrl } from './site'
...
const DEFAULT_DESCRIPTION = 'JovHub — a fast skill registry for agents, with vector search.'
const DEFAULT_SOUL_DESCRIPTION = 'JovSouls — the home for SOUL.md bundles and personal system lore.'
const OG_SKILL_IMAGE_LAYOUT_VERSION = '5'
const OG_SOUL_IMAGE_LAYOUT_VERSION = '1'

export function getSiteUrl() {
  return getJovHubSiteUrl()
}
...
export function buildSkillMeta(source: SkillMetaSource): SkillMeta {
  ...
  const title = `${displayName} — JovHub`
  const description =
    summary || (owner ? `Agent skill by @${owner} on JovHub.` : DEFAULT_DESCRIPTION)
...
export function buildSoulMeta(source: SoulMetaSource): SoulMeta {
  ...
  const summary = clean(source.summary)
  const version = clean(source.version)
  const title = `${displayName} — JovSouls`
  const description =
    summary || (owner ? `Soul by @${owner} on JovSouls.` : DEFAULT_SOUL_DESCRIPTION)
