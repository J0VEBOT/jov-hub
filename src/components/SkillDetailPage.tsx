...
import type { JovisSkillMetadata, SkillInstallSpec } from 'jovhub-schema'
...
function JOVIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      width="1em"
  ...
  if (variant === 'badge') {
    return (
      <>
        ...
        {llmStatusInfo ? (
          <div className="version-scan-badge">
            <JOVIcon className="version-scan-icon version-scan-icon-oc" />
    ...
    <div className="scan-results-panel">
      ...
      <div className="scan-results-list">
        ...
        {llmStatusInfo && llmAnalysis ? (
          <div className="scan-result-row">
            <div className="scan-result-scanner">
              <JOVIcon className="scan-result-icon scan-result-icon-oc" />
              <span className="scan-result-scanner-name">JOV</span>
            </div>
...
export function SkillDetailPage({
  ...
  const jovis = (latestVersion?.parsed as { jovis?: JovisSkillMetadata } | undefined)?.jovis
  const osLabels = useMemo(() => formatOsList(jovis?.os), [jovis?.os])
  const requirements = jovis?.requires
  const installSpecs = jovis?.install ?? []
  const nixPlugin = jovis?.nix?.plugin
  const nixSystems = jovis?.nix?.systems ?? []
  const nixSnippet = nixPlugin ? formatNixInstallSnippet(nixPlugin) : null
  const configRequirements = jovis?.config
  const configExample = configRequirements?.example
    ? formatConfigSnippet(configRequirements.example)
    : null
  const cliHelp = jovis?.cliHelp
        ...
        {modInfo?.isPendingScan ? (
          ...
          <div className="pending-banner pending-banner-blocked">
            <div className="pending-banner-content">
              ...
              <p>
                JovHub Security flagged this skill as malicious. Downloads are disabled. Review the
                scan results below.
              </p>
            </div>
          </div>
          ...
          <div className="pending-banner pending-banner-warning">
            <div className="pending-banner-content">
              ...
              <p>
                JovHub Security flagged this skill as suspicious. Review the scan results before
                using.
              </p>
              {canManage ? (
                <p className="pending-banner-appeal">
                  ...
                  <a
                    href="https://github.com/J0VEBOT/jovhub/issues"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
              ...
              <div className="skill-hero-title">
                ...
                {latestVersion?.sha256hash || latestVersion?.llmAnalysis ? (
                  <p className="scan-disclaimer">
                    Like a bolt shell, security has layers — review code before you run it.
                  </p>
                ) : null}
              </div>
                ...
                {hasRuntimeRequirements ? (
                  <div className="skill-panel">
                    ...
                    <div className="skill-panel-body">
                      {jovis?.emoji ? <div className="tag">{jovis.emoji} JOVis</div> : null}
                      {osLabels.length ? (
                        <div className="stat">
                          <strong>OS</strong>
        ...
        {nixSnippet ? (
          <div className="card">
            ...
            <p className="section-subtitle" style={{ margin: 0 }}>
              {nixSystems.length ? `Systems: ${nixSystems.join(', ')}` : 'nix-jovbot'}
            </p>
...
function formatNixInstallSnippet(plugin: string) {
  const snippet = `programs.jovbot.plugins = [ { source = "${plugin}"; } ];`
