...
function getMonacoThemeName() {
  if (typeof document === 'undefined') return 'jovhub-light'
  return document.documentElement.dataset.theme === 'dark' ? 'jovhub-dark' : 'jovhub-light'
}
...
function applyMonacoTheme(monaco: NonNullable<ReturnType<typeof useMonaco>>) {
  ...
  monaco.editor.defineTheme(`jovhub-${isDark ? 'dark' : 'light'}`, {
    base,
    inherit: true,
    rules: [
  ...
  monaco.editor.setTheme(`jovhub-${isDark ? 'dark' : 'light'}`)
}
