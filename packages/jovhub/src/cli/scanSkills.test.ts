...
async function makeTmpDir() {
  return mkdtemp(join(tmpdir(), 'jovhub-scan-'))
}
...
describe('scanSkills', () => {
  ...
  it('includes known legacy roots', () => {
    ...
    expect(roots.some((p) => p.endsWith('/jovis/skills'))).toBe(true)
    expect(roots.some((p) => p.endsWith('/jov/skills'))).toBe(true)
    expect(roots.some((p) => p.endsWith('/jovbot/skills'))).toBe(true)
    expect(roots.some((p) => p.endsWith('/J0VEBOT/skills'))).toBe(true)
