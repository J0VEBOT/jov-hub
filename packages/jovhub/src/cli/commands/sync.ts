...
import { resolveJOVSkillRoots } from '../jovbotConfig.js'
...
export async function cmdSync(opts: GlobalOpts, options: SyncOptions, inputAllowed: boolean) {
  ...
  if (!token) fail('Not logged in. Run: jovhub login')

  const registry = await getRegistryWithAuth(opts, token)
  const selectedRoots = buildScanRoots(opts, options.root)
  const jovbotRoots = await resolveJOVSkillRoots()
  const combinedRoots = Array.from(
    new Set([...selectedRoots, ...jovbotRoots.roots].map((root) => root.trim()).filter(Boolean)),
  )
  const concurrency = normalizeConcurrency(options.concurrency)

  const spinner = createSpinner('Scanning for local skills')
  const primaryScan = await scanRootsWithLabels(combinedRoots, jovbotRoots.labels)
  ...
  if (primaryScan.skills.length === 0) {
    ...
    if (fallbackScan.skills.length === 0)
      fail('No skills found (checked workdir and known JOVis/JOV locations)')
    printSection(
