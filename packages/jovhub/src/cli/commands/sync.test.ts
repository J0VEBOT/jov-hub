...
const mockResolveJOVSkillRoots = vi.fn(
  async () =>
    ({
      roots: [] as string[],
...
vi.mock('../jovbotConfig.js', () => ({
  resolveJOVSkillRoots: () => mockResolveJOVSkillRoots(),
}))
...
describe('cmdSync', () => {
  ...
  it('prints labeled roots when jovbot roots are detected', async () => {
    interactive = false
    mockResolveJOVSkillRoots.mockResolvedValueOnce({
      roots: ['/auto'],
