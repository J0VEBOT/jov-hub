...
import { resolveJOVDefaultWorkspace } from './cli/jovbotConfig.js'
...
const program = new Command()
  .name('jovhub')
  .description(
    `${styleTitle(`JovHub CLI ${getCliBuildLabel()}`)}\n${styleEnvBlock(
...
async function resolveWorkdir(explicit?: string) {
  ...
  const jovbotWorkspace = await resolveJOVDefaultWorkspace()
  return jovbotWorkspace ? resolve(jovbotWorkspace) : cwd
}

async function hasClawhubMarker(workdir: string) {
  const lockfile = join(workdir, '.jovhub', 'lock.json')
  if (await pathExists(lockfile)) return true
  const markerDir = join(workdir, '.jovhub')
  if (await pathExists(markerDir)) return true
  const legacyLockfile = join(workdir, '.jovhub', 'lock.json')
  if (await pathExists(legacyLockfile)) return true
  const legacyMarkerDir = join(workdir, '.jovhub')
