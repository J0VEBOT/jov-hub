...
describe('skills', () => {
  it('extracts zip into directory and skips traversal', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'jovhub-'))
    const zip = zipSync({
  ...
  it('writes and reads lockfile', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-work-'))
  ...
  it('returns empty lockfile on invalid json', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-work-bad-'))
    await mkdir(join(workdir, '.jovhub'), { recursive: true })
    await writeFile(join(workdir, '.jovhub', 'lock.json'), '{', 'utf8')
    const read = await readLockfile(workdir)
    expect(read).toEqual({ version: 1, skills: {} })
  })

  it('returns empty lockfile on schema mismatch', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-work-schema-'))
    await mkdir(join(workdir, '.jovhub'), { recursive: true })
    await writeFile(
      join(workdir, '.jovhub', 'lock.json'),
  ...
  it('skips dotfiles and node_modules when listing text files', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-files-'))
  ...
  it('respects .gitignore and .jovhubignore', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-ignore-'))
    await writeFile(join(workdir, '.gitignore'), 'ignored.md\n', 'utf8')
    await writeFile(join(workdir, '.jovhubignore'), 'private.md\n', 'utf8')
    await writeFile(join(workdir, 'SKILL.md'), 'hi', 'utf8')
  ...
  it('falls back to text/plain for unknown text extensions', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-env-'))
    await writeFile(join(workdir, 'SKILL.md'), 'hi', 'utf8')
  ...
  it('returns null for invalid skill origin metadata', async () => {
    const workdir = await mkdtemp(join(tmpdir(), 'jovhub-origin-'))
    expect(await readSkillOrigin(workdir)).toBeNull()

    await mkdir(join(workdir, '.jovhub'), { recursive: true })
    await writeFile(
      join(workdir, '.jovhub', 'origin.json'),
